            ...
            if (p == EMPTY) {
                ...
                for (int j = 0; j < pieces_point; ++j) {
                    ...
                    if (can_move_to_square(b, piece_squares[j], temp)
                            ...
                            && !would_be_weak_after_move(gs, temp, Move{piece_squares[j], temp})) {
        ...
        if (val(defend_from_square) && b.get(defend_from_square) == EMPTY) {
            ...
            for (int j = 0; j < 8; ++j) {
                if (val(temp = mksq(x + XKN[i] + XKN[j], y + YKN[i] + YKN[j]))
                        ...
                        && (temp != s)
                        && !would_be_weak_after_move(gs, defend_from_square, Move{temp, defend_from_square})) {
                    if (move_counter.has_space()) {
    ...
    for (int i = 0; i < pieces_point; ++i) {
        ...
        if (type(b.get(piece_squares[i])) == PAWN) {
            if (val(left_pawn_defence_square)
                    ...
                    && !would_be_weak_after_move(gs, left_pawn_defence_square, Move{piece_squares[i], left_pawn_defence_square})) {
                if (move_counter.has_space()) {
                    ...
                    return;
                        ...
                        && !would_be_weak_after_move(gs, right_pawn_defence_square, Move{piece_squares[i], right_pawn_defence_square})) {
                if (move_counter.has_space()) {
